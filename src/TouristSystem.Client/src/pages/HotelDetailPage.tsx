...
import type { IHotel, IReview } from '../types';
import { useAuthStore } from '../app/store';
import { useTranslation } from '../hooks/useTranslation';
import { MapPin, Star, Heart, Calendar, ArrowRight, Loader2, MessageSquare, Plus, Sparkles, Coffee } from 'lucide-react';
...
export default function HotelDetailPage() {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      const reviewsRes = await api.get('/reviews', {
        params: { reviewType: 'Hotel', referenceId: id },
      });
  ...
  const imagePlaceholder = 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1200&q=80';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative rounded-[32px] overflow-hidden h-[450px] w-full mb-12 shadow-2xl">
          ...
          <div className="absolute bottom-8 left-8 right-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 text-white z-10">
            <div className="space-y-3 font-light text-left">
              ...
              <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight leading-tight">{getLocalized(hotel, 'name')}</h1>
            </div>
            ...
            <div className="bg-slate-900/40 border border-slate-900/60 p-8 rounded-3xl backdrop-blur-xl shadow-xl space-y-6 text-left">
              <div className="flex items-center justify-between">
                <h2 className="text-2xl font-extrabold text-white flex items-center space-x-2">
                  ...
                  <span>{t('hotels.aboutHotel')}</span>
                </h2>
                <div className="flex items-center space-x-2">
                  <span className="text-xs bg-blue-500/10 border border-blue-500/30 text-sky-400 px-3.5 py-1.5 rounded-full font-bold">
                    {hotel.stars} {t('common.stars')} ★
                  </span>
                  <span className="text-xs text-slate-400 font-light">
                    ({formatNumber(hotel.availableRooms)} {t('hotels.availableRooms')})
                  </span>
                </div>
              </div>
              ...
              {hotel.amenities && (
                <div className="pt-6 border-t border-slate-800/40">
                  <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center space-x-1.5">
                    ...
                    <span>{t('hotels.amenities')}</span>
                  </h3>
                  <div className="flex flex-wrap gap-2.5">
                    {hotel.amenities.split(',').map((amenity, index) => (
                      <span key={index} className="bg-slate-950 border border-slate-800 text-slate-300 text-xs px-4 py-2 rounded-2xl font-light">
                        {amenity.trim()}
                      </span>
                    ))}
                  </div>
          ...
          <div className="lg:col-span-1 space-y-8">
            ...
            <div className="bg-slate-900/40 border border-slate-900/60 p-8 rounded-3xl backdrop-blur-xl shadow-xl space-y-6">
              <h3 className="text-xl font-extrabold text-white flex items-center space-x-2 pb-4 border-b border-slate-800/40">
                ...
                <span>{t('hotels.bookingForm')}</span>
              </h3>
              ...
              {bookingSuccess ? (
                ...
                <form onSubmit={handleBookingSubmit} className="space-y-4 text-left">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 ml-2">{t('hotels.checkIn')}</label>
                      <input
                        type="date"
                        value={startDate}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 ml-2">{t('hotels.checkOut')}</label>
                      <input
                        type="date"
                        value={endDate}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 ml-2">{t('common.quantity')}</label>
                    <input
                      type="number"
                      min={1}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 ml-2">{t('hotels.guestsCount')}</label>
                    <input
                      type="number"
                      min={1}
                      value={guestsCount}
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2 ml-2">{t('common.notes')}</label>
                    <textarea
                      rows={3}
                      value={notes}
                  ...
                  <div className="py-4 border-t border-slate-800/40 flex items-center justify-between">
                    <span className="text-xs text-slate-400 uppercase font-semibold tracking-wider">{t('hotels.pricePerNight')}</span>
                    <span className="text-xl font-extrabold text-blue-500 dark:text-sky-400">
                      {formatCurrency(hotel.pricePerNight)} <span className="text-xs font-normal text-slate-500">/ {t('common.perNight')}</span>
                    </span>
                  </div>
                  ...
                  {user ? (
                    ...
                    >
                      {bookingLoading ? (
                        ...
                        <>
                          <span>{t('hotels.confirmBooking')}</span>
                    ...
                    <div className="text-center py-4 bg-slate-950 border border-slate-900 rounded-2xl space-y-4">
                      <p className="text-xs text-slate-400 font-light px-4">{t('hotels.loginToBook')}</p>
