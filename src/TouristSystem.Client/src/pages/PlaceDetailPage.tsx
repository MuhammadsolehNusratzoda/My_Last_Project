...
import type { IPlace, IReview } from '../types';
import { useAuthStore } from '../app/store';
import { useTranslation } from '../hooks/useTranslation';
import { MapPin, Star, Heart, MessageSquare, Plus, Loader2, Sparkles, Navigation } from 'lucide-react';
...
export default function PlaceDetailPage() {
  ...
  const fetchDetails = async () => {
    ...
    try {
      ...
      const reviewsRes = await api.get('/reviews', {
        params: { reviewType: 'Place', referenceId: id },
      });
  ...
  const handleReviewSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setComment('');
      setRating(5);
      fetchDetails(); // Reload to refresh score
    } catch (err: any) {
  ...
  const imagePlaceholder = 'https://images.unsplash.com/photo-1507525428034-b723cf961d3e?auto=format&fit=crop&w=1200&q=80';
        ...
        <div className="relative rounded-[32px] overflow-hidden h-[450px] w-full mb-12 shadow-2xl">
          ...
          <div className="absolute bottom-8 left-8 right-8 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 text-white z-10">
            <div className="space-y-3">
              ...
              <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight leading-tight">{getLocalized(place, 'name')}</h1>
            </div>
          ...
          <div className="lg:col-span-2 space-y-10">
            ...
            <div className="bg-slate-900/40 border border-slate-900/60 p-8 rounded-3xl backdrop-blur-xl shadow-xl space-y-6 text-left">
              <h2 className="text-2xl font-extrabold text-white flex items-center space-x-2">
                ...
                <span>{t('places.aboutLocation')}</span>
              </h2>
              ...
              {place.coordinates && (
                <div className="flex items-center space-x-2 text-xs text-slate-400 bg-slate-950 border border-slate-900 p-4 rounded-2xl w-max">
                  <Navigation className="h-4.5 w-4.5 text-blue-500" />
                  <span className="font-bold text-slate-300 uppercase tracking-wider">{t('places.coordinates')}:</span>
